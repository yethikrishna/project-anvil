...
type SearchTab = 'all' | 'images' | 'chat' | 'news';
...
function ChatPanel({ initialQuery, searchApi }: { initialQuery: string; searchApi: string }) {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'init',
      role: 'assistant',
      content: `I found results for "${initialQuery}". Ask me follow-up questions about these results.`,
    },
  ]);
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${searchApi}/api/chat`, {
        method: 'POST',
      ...
      const errorMsg: ChatMessage = {
        id: `error-${Date.now()}`,
        role: 'assistant',
        content: 'Sorry, I couldn\'t process that. The chat service may not be configured. Make sure OPENAI_API_KEY is set.',
      };
      setMessages(prev => prev.filter(m => !m.loading).concat(errorMsg));
    } finally {
      setSending(false);
    }
  }, [input, sending, messages, initialQuery, searchApi]);
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-3">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 border border-gray-300 dark:border-gray-600 rounded-full px-4 py-2 text-sm outline-none focus:border-blue-500 dark:bg-gray-800 dark:text-white"
  ...
  if (!searched) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center pt-32">
        ...
        <p className="text-gray-400 text-sm mb-8">Hybrid BM25 + Semantic Search + Conversational AI</p>
      ...
      <div className="px-6 py-4 max-w-5xl">
        ...
        {tab === 'chat' && (
          <div className="max-w-3xl">
            <ChatPanel initialQuery={query} searchApi={SEARCH_API} />
          </div>
        )}
